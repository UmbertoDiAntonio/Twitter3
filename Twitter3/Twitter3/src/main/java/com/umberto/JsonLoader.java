...
public class JsonLoader {
    public JsonLoader() throws IOException {
        ...
        for (int i = 0; i < statuses.size(); i++) {
            ...
            UserData.getUserDataList().add(new UserData(screenName, message, createdAt));
        }
        for (UserData user : UserData.getUserDataList()
        ) {
            UserData.getScreenListExt().add(user.getScreenName());
        }
        UserData.getScreenListExt().add(0, "ALL");
    }
}
